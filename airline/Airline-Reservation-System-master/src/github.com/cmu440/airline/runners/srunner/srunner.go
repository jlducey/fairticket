...
func main() {
	...
	// If nodeID is 0, then assign a random integer instead.
	randID := int(*nodeID)
	if randID == 0 {
	...
	fmt.Print("Request to open StorageServer on port: ")
	fmt.Println(*port)
	_, err := storageserver.NewStorageServer(*masterHostPort, *numNodes, *port, randID)
	

	if err != nil {
		fmt.Println("Failed to create storage server:", err)
		return
	} else{
		fmt.Print("StorageServer is now listening on port: ")
		fmt.Println(*port)
	}
