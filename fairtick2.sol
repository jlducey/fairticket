...
contract FTT is ERC20, SafeMath, Ownable {
  ...
  modifier onlyUnlocked() {
    if (msg.sender != owner && locked) revert();
    _;
  }
...
// mincap failed to be met.. need to refund..guessing?
	modifier minCapNotReached() {
		if ((now<endBlock) || FTTSentToETH + FTTSentToBTC >= minCap ) revert();
		_;
	}
...
// don't process payments outside timeframe based on blocks
	...
	*/
	...
	function Crowdsale() {
		...
		minCap=2000000000;
		maxCap=6000000000;
    	 	ftt_bounty= 170000000;	// max 6000000 FTT
		ftt_reserve=170000000;	// max 6000000 FTT
		ftt_team=12000000;
	}
	...
	*/
	function receiveETH(address beneficiary) internal stopInEmergency  respectTimeFrame  {
		if (msg.value < minInvestETH) revert();								//don't accept funding under a predefined threshold
		uint fttToSend = bonus(safeMul(msg.value,FTTPerETH)/(1 ether));					//compute the number of FTT to send
		if (safeAdd(fttToSend, safeAdd(FTTSentToETH, FTTSentToBTC)) > maxCap) revert();	
		Backer backer = backers[beneficiary];
		if (!ftt.transfer(beneficiary, fttToSend)) revert(); 			// Do the FTT transfer right now 
	...
	*/
	function receiveBTC(address beneficiary, string btc_address, uint value, string txid) stopInEmergency respectTimeFrame onlyBy(BTCproxy) returns (bool res){
		if (value < minInvestBTC) revert();								// this verif is also made on the btcproxy
		...
		Backer backer = backers[beneficiary];
		if (!ftt.transfer(beneficiary, fttToSend)) revert();							// Do the transfer right now 
		backer.fttSent = safeAdd(backer.fttSent , fttToSend);
	...
	function receiveApproval(address _from, uint256 _value, address _token, bytes _extraData) minCapNotReached public {
		if (msg.sender != address(ftt)) revert(); 
		if (_extraData.length != 0) revert();						// no extradata needed
		if (_value != backers[_from].fttSent) revert();					// compare value from backer balance
		if (!ftt.transferFrom(_from, address(this), _value)) revert() ;			// get the token back to the crowdsale contract
		if (!ftt.burn(_value)) revert() ;							// token sent for refund are burnt
	...
	*/
	function finalize() onlyBy(owner) {
		// check
		if (FTTSentToETH + FTTSentToBTC < maxCap - 5000000000000 && now < endBlock) revert();	// cannot finalise before 30 day until maxcap is reached minus 1BTC
		if (FTTSentToETH + FTTSentToBTC < minCap && now < endBlock + 15 days) revert() ;		// if mincap is not reached donors have 15days to get refund before we can finalise
		if (!multisigETH.send(this.balance)) revert();						// moves the remaining ETH to the multisig address
		if (ftt_reserve > 6000000000000000){							// moves FTT to the team, reserve and bounty address
			if(!ftt.transfer(reserve,6000000000000000)) revert();				// max cap 6000000FTT
			ftt_reserve = 6000000000000000;
		} else {
			if(!ftt.transfer(reserve,ftt_reserve)) revert();  
		}
		if (ftt_bounty > 6000000000000000){
			if(!ftt.transfer(bounty,6000000000000000)) revert();				// max cap 6000000FTT
			ftt_bounty = 6000000000000000;
		} else {
			if(!ftt.transfer(bounty,ftt_bounty)) revert();
		}
		if (!ftt.transfer(team,ftt_team)) revert();
